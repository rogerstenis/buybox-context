...
- `sortStrategy` enum:
  ...
  | `protocol` | `string` | Sort sellers using a protocol defined on [buybox-graphql](https://github.com/vtex-apps/buybox-graphql). This protocol defines the contract to receive and response data. So it works, it's necessary to implement a resolver for this contracts. In this case, was created the [buybox-resolver](https://github.com/vtex-apps/buybox-resolver) as example. |
